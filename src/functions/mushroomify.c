...
Kernel create_mushroomify_kernel(int strength) {
    ...
    mushroomify_kernel.kernel_size = 20;

    double outer_points = mushroomify_kernel.kernel_size * mushroomify_kernel.kernel_size - 1;

    double center_value = 1.0 + (strength / 10.0);
    double outer_value = -1.0 * (center_value - 1)/outer_points;

    int center = mushroomify_kernel.kernel_size / 2;

    mushroomify_kernel.kernel_array = malloc(mushroomify_kernel.kernel_size * sizeof(*mushroomify_kernel.kernel_array));
    for (int y = 0; y < mushroomify_kernel.kernel_size; y++) {
        mushroomify_kernel.kernel_array[y] = malloc(mushroomify_kernel.kernel_size * sizeof(double));
        for (int x = 0; x < mushroomify_kernel.kernel_size; x++) {
